...
static void increment_gru_layer(layer *l, int steps)
{
...
void forward_gru_layer(layer l, network_state state)
{
    ...
    for (i = 0; i < l.steps; ++i) {
		...
		increment_gru_layer(&input_z_layer, 1);
		increment_gru_layer(&input_r_layer, 1);
		increment_gru_layer(&input_h_layer, 1);

		increment_gru_layer(&state_z_layer, 1);
		increment_gru_layer(&state_r_layer, 1);
		increment_gru_layer(&state_h_layer, 1);
    }
}
...
void backward_gru_layer_gpu(layer l, network_state state)
{
	...
	increment_gru_layer(&input_z_layer, l.steps - 1);
	increment_gru_layer(&input_r_layer, l.steps - 1);
	increment_gru_layer(&input_h_layer, l.steps - 1);

	increment_gru_layer(&state_z_layer, l.steps - 1);
	increment_gru_layer(&state_r_layer, l.steps - 1);
	increment_gru_layer(&state_h_layer, l.steps - 1);
