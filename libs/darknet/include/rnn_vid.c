...
float_pair get_rnn_vid_data(network net, char **files, int n, int batch, int steps)
{
    ...
    for(b = 0; b < batch; ++b){
        ...
        for(i = 0; i < net.batch; ++i){
            ...
            //cvWaitKey(10);
            memcpy(input + i*input_size, re.data1, input_size*sizeof(float));
            free_image(im);
            free_image(re);
        }
...
void train_vid_rnn(char *cfgfile, char *weightfile)
{
    ...
    list1 *plist = get_paths(train_videos);
    int N = plist->size;
...
void generate_vid_rnn(char *cfgfile, char *weightfile)
{
    ...
    for(i = 0; i < 25; ++i){
        ...
        feat = network_predict(extractor, re.data1);
        if(i > 0){
    ...
    for(i = 0; i < 30; ++i){
        ...
        image new1 = save_reconstruction(extractor, &last, next, "new", i);
        free_image(last);
        last = new1;
    }
}
