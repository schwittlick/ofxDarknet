...
void train_classifier(char *datacfg, char *cfgfile, char *weightfile, int *gpus, int ngpus, int clear)
{
    ...
    list1 *options = read_data_cfg(datacfg);
    ...
    list1 *plist = get_paths(train_list);
...
void validate_classifier_crop(char *datacfg, char *filename, char *weightfile)
{
    ...
    char **labels = get_labels(label_list);
    list1 *plist = get_paths(valid_list);
...
void validate_classifier_10(char *datacfg, char *filename, char *weightfile)
{
    ...
    for(i = 0; i < m; ++i){
        ...
        for(j = 0; j < 10; ++j){
            float *p = network_predict(net, images[j].data1);
...
void validate_classifier_full(char *datacfg, char *filename, char *weightfile)
{
    ...
    for(i = 0; i < m; ++i){
        ...
        //cvWaitKey(0);
        float *pred = network_predict(net, resized.data1);
...
void validate_classifier_single(char *datacfg, char *filename, char *weightfile)
{
    ...
    for(i = 0; i < m; ++i){
        ...
        //show_image(crop, "cropped");
        //cvWaitKey(0);
        float *pred = network_predict(net, crop.data1);
        if(net.hierarchy) hierarchy_predictions(pred, net.outputs, net.hierarchy, 1);

        if(resized.data1 != im.data1) free_image(resized);
        free_image(im);
        free_image(crop);
        top_k(pred, classes, topk, indexes);
...
void validate_classifier_multi(char *datacfg, char *filename, char *weightfile)
{
    ...
    for(i = 0; i < m; ++i){
        ...
        for(j = 0; j < nscales; ++j){
            ...
            float *p = network_predict(net, r.data1);
            if(net.hierarchy) hierarchy_predictions(p, net.outputs, net.hierarchy, 1);
            axpy_cpu(classes, 1, p, 1, pred, 1);
            flip_image(r);
            p = network_predict(net, r.data1);
            axpy_cpu(classes, 1, p, 1, pred, 1);
            if(r.data1 != im.data1) free_image(r);
        }
        free_image(im);
        top_k(pred, classes, topk, indexes);
        free(pred);
...
void try_classifier(char *datacfg, char *cfgfile, char *weightfile, char *filename, int layer_num)
{
    ...
    while(1){
        ...
        normalize_cpu(im.data1, mean, var, 1, 3, im.w*im.h);
...
void label_classifier(char *datacfg, char *filename, char *weightfile)
{
    ...
    char **labels = get_labels(label_list);
    list1 *plist = get_paths(test_list);
...
void threat_classifier(char *datacfg, char *cfgfile, char *weightfile, int cam_index, const char *filename)
{
    ...
    while(1){
        ...
        float *predictions = network_predict(net, in_s.data1);
        float curr_threat = 0;
        if(1){
