...
void train_dice(char *cfgfile, char *weightfile)
{
    ...
    list1 *plist = get_paths("data/dice/dice.train.list");
    ...
    while(1){
        ++i;
        time=clock();
        data1 train = load_data_old(paths, imgs, plist->size, labels, 6, net.w, net.h);
...
void validate_dice(char *filename, char *weightfile)
{
    ...
    char **labels = dice_labels;
    list1 *plist = get_paths("data/dice/dice.val.list");

    char **paths = (char **)list_to_array(plist);
    int m = plist->size;
    free_list(plist);

    data1 val = load_data_old(paths, m, 0, labels, 6, net.w, net.h);
