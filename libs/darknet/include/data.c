...
{
    list1 *plist = get_paths(filename);
...
data1 load_data_region(int n, char **paths, int m, int w, int h, int size, int classes, float jitter, float hue, float saturation, float exposure)
{
...
data1 load_data_compare(int n, char **paths, int m, int classes, int w, int h)
{
    ...
    for(i = 0; i < n; ++i){
        ...
        memcpy(d.X.vals[i],         im1.data1, h*w*3*sizeof(float));
        memcpy(d.X.vals[i] + h*w*3, im2.data1, h*w*3*sizeof(float));
...
data1 load_data_swag(char **paths, int n, int classes, float jitter)
{
    int index = rand()%n;
...
void *load_threads(void *ptr)
{
    ...
    int total = args.n;
    free(ptr);
    data1 *buffers = (data1*)calloc(args.threads, sizeof(data1));
    pthread_t *threads = (pthread_t*)(args.threads, sizeof(pthread_t));
...
data1 load_data_super(char **paths, int n, int m, int w, int h, int scale)
{
    ...
    for(i = 0; i < n; ++i){
        ...
        d.X.vals[i] = resize.data1;
        d.y.vals[i] = crop.data1;
        free_image(im);
    }
...
data1 concat_datas(data1 *d, int n)
{
    ...
    data1 out = {0};
    for(i = 0; i < n; ++i){
        data1 new1 = concat_data(d[i], out);
        free_data(out);
        out = new1;
    }
    return out;
...
{
    data1 *split = (data1*)calloc(2, sizeof(data1));
    int i;
