...
void test_cifar_multi(char *filename, char *weightfile)
{
    ...
    float avg_acc = 0;
    data1 test = load_cifar10_data("data/cifar/cifar-10-batches-bin/test_batch.bin");
    ...
    for(i = 0; i < test.X.rows; ++i){
        ...
        float *p = network_predict(net, im.data1);
        axpy_cpu(10, 1, p, 1, pred, 1);
        flip_image(im);
        p = network_predict(net, im.data1);
        axpy_cpu(10, 1, p, 1, pred, 1);
