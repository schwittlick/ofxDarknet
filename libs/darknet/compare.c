...
void train_compare(char *cfgfile, char *weightfile)
{
    ...
    int imgs = 1024;
    list1 *plist = get_paths("data/compare.train.list");
...
void validate_compare(char *filename, char *weightfile)
{
    ...
    list1 *plist = get_paths("data/compare.val.list");
...
int bbox_comparator(const void *a, const void *b)
{
    ...
    memcpy(X,                   im1.data1, im1.w*im1.h*im1.c*sizeof(float));
    memcpy(X+im1.w*im1.h*im1.c, im2.data1, im2.w*im2.h*im2.c*sizeof(float));
    float *predictions = network_predict(net, X);
    
    free_image(im1);
    free_image(im2);
    free(X);
    if (predictions[class1*2] > predictions[class1*2+1]){
        return 1;
    }
    return -1;
}

void bbox_update(sortable_bbox *a, sortable_bbox *b, int class1, int result)
{
    int k = 32;
    float EA = 1./(1+pow(10, (b->elos[class1] - a->elos[class1])/400.));
    float EB = 1./(1+pow(10, (a->elos[class1] - b->elos[class1])/400.));
    float SA = result ? 1 : 0;
    float SB = result ? 0 : 1;
    a->elos[class1] += k*(SA - EA);
    b->elos[class1] += k*(SB - EB);
}

void bbox_fight(network net, sortable_bbox *a, sortable_bbox *b, int classes, int class1)
{
...
void SortMaster3000(char *filename, char *weightfile)
{
    ...
    list1 *plist = get_paths("data/compare.sort.list");
    //list *plist = get_paths("data/compare.val.old");
    char **paths = (char **)list_to_array(plist);
    int N = plist->size;
    free_list(plist);
...
void BattleRoyaleWithCheese(char *filename, char *weightfile)
{
    ...
    for (class1 = 0; class1 < classes; ++class1){
        ...
        for(round = 1; round <= 100; ++round){
            ...
            for(i = 0; i < N/2; ++i){
                bbox_fight(net, boxes+i*2, boxes+i*2+1, classes, class1);
            }
            qsort(boxes, N, sizeof(sortable_bbox), elo_comparator);
        ...
        char buff[256];
        sprintf(buff, "results/battle_%d.log", class1);
        FILE *outfp = fopen(buff, "w");
        for(i = 0; i < N; ++i){
            fprintf(outfp, "%s %f\n", boxes[i].filename, boxes[i].elos[class1]);
        }
        fclose(outfp);
    }
