...
void optimize_picture(network *net, image orig, int max_layer, float scale, float rate, float thresh, int norm)
{
...
#ifdef GPU
    state.input = cuda_make_array(im.data1, im.w*im.h*im.c);
    state.delta = cuda_make_array(im.data1, im.w*im.h*im.c);
    ...
    cuda_pull_array(state.delta, delta.data1, im.w*im.h*im.c);
    cuda_free(state.input);
    cuda_free(state.delta);
#else
    state.input = im.data1;
    state.delta = delta.data1;
    ...
    if(norm) normalize_array(out.data1, out.w*out.h*out.c);
    axpy_cpu(orig.w*orig.h*orig.c, rate, out.data1, 1, orig.data1, 1);
            ...
            for(i = 0; i < recon.w; ++i){
                ...
                for(jj = j-num; jj <= j + num && jj < recon.h; ++jj){
                    ...
                    for(ii = i-num; ii <= i + num && ii < recon.w; ++ii){
                        ...
                        update.data1[out_index] += lambda * (recon.data1[in_index] - recon.data1[out_index]);
                    }
                }
            }
        }
...
void reconstruct_picture(network net, float *features, image recon, image update, float rate, float momentum, float lambda, int smooth_size, int iters)
{
    ...
    for (iter = 0; iter < iters; ++iter) {
        ...
        network_state state = {0};
#ifdef GPU
        state.input = cuda_make_array(recon.data1, recon.w*recon.h*recon.c);
        state.delta = cuda_make_array(delta.data1, delta.w*delta.h*delta.c);
        state.truth = cuda_make_array(features, get_network_output_size(net));

        forward_network_gpu(net, state);
        backward_network_gpu(net, state);

        cuda_pull_array(state.delta, delta.data1, delta.w*delta.h*delta.c);

        cuda_free(state.input);
        cuda_free(state.delta);
        cuda_free(state.truth);
#else
        state.input = recon.data1;
        state.delta = delta.data1;
        ...
        axpy_cpu(recon.w*recon.h*recon.c, 1, delta.data1, 1, update.data1, 1);
        smooth(recon, update, lambda, smooth_size);

        axpy_cpu(recon.w*recon.h*recon.c, rate, update.data1, 1, recon.data1, 1);
        scal_cpu(recon.w*recon.h*recon.c, momentum, update.data1, 1);
